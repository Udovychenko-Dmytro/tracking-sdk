...
Conventions inferred from the existing `Packages/com.dmytroudovychenko.tracking` source, plus the always-on rules in [CLAUDE.md](../../CLAUDE.md). This SDK is **plain-C#-first with DI seams**, not a MonoBehaviour-heavy Unity gameplay codebase. Naming follows the MSNow house style (`m_` fields, `UPPER_SNAKE_CASE` consts, explicit-index enums); it still diverges from typical Unity game code on namespacing, async-not-coroutines, and `ITrackingLogger` (see the call-out).
...
> ⚠️ **House-style (MSNow conventions) — applied across the package.** The SDK uses the MSNow house style for **field naming, constant naming, enums, and local-variable typing**; `com.dmytroudovychenko.tracking` + tests were migrated to match and stay green on the EditMode suite. The rules:

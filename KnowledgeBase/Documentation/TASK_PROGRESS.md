...
- ✅ `Packages/com.dmytroudovychenko.tracking/Documentation~/track.php` optional receiver example
  (validate → log → `200 {ok,received}`); deploy notes folded into `Documentation~/index.md`.
...
- ✅ `[Category("Live")]` `LiveTransportTests` for the real endpoint — runs in the default suite (POSTs to the live receiver, so the run needs network); the category lets it be filtered out when offline.

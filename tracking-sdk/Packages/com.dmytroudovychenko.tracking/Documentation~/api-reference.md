...
public TrackingSystem(
    ...
    IDeadLetterSink      deadLetter   = null   // default: bounded DeadLetterQueue
)
```
...
{
    DropOldest = 0,   // Evict the oldest event to make room (default)
    RejectNew  = 1    // Reject the incoming event
}
```
...
{
    Simulated = 0,   // In-process, no network
    Http      = 1    // Real HTTP POST
}
```
...
{
    Debug   = 0,   // Most verbose — per-step traces, event payloads
    Info    = 1,
    Warning = 2,   // Default MinLogLevel
    Error   = 3    // Least verbose — errors only
}
```
...
Key fields: `EventsEnqueued`, `EventsDelivered`, `EventsFailed`, `EventsDropped`,
`EventsDeadLettered`, `BatchesSent`, `RetriesAttempted`, `CircuitBreakerTrips`,
`PersistenceWrites`, `PersistenceLoads`.
...
{
    int Count { get; }
    IReadOnlyList<TrackingEvent> Events { get; }
    void Clear();
}
```
...
| `IDeadLetterSink` | `DeadLetterQueue` | Give-up event storage |

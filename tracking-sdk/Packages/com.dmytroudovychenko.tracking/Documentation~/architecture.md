...
1000). Inspect via `Tracker.DeadLetter.Events`, clear via `Tracker.DeadLetter.Clear()`. Events in
...
| `idempotencyKey` | Stable GUID per event (for server-side dedup) |
...
the response was lost). Each event carries a stable `idempotencyKey` for server-side deduplication.

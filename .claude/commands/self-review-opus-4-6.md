---
description: DEEP self-review (whole project diff base..HEAD + uncommitted) by a fresh Claude (Opus 4.6 — previous-generation second opinion, clean context, --effort max), then autopilot the review -> fix -> re-review -> verify loop. Verification = the headless Unity EditMode test run.
argument-hint: "[base-ref] (optional; defaults to the repo's initial commit — i.e. review the whole SDK)"
allowed-tools: Bash, Read, Edit, Write, Grep, Glob
---

# /self-review-opus-4-6 — DEEP self-review by a fresh Claude (Opus 4.6)

You run a **fully autonomous** self-review loop. The external reviewer is a **fresh Claude (Opus 4.6)** in a separate process with clean context (`claude -p --model claude-opus-4-6 --effort max`). You are the developer: you receive the review, fix, ask for a re-check, verify, and finish.

This is the **previous-generation** sibling of `/self-review` (Opus 4.8). Why a separate process with a pinned older model: clean context (no attachment to the code you just wrote) **plus** a second opinion from a different model generation — Opus 4.6 catches things the same-generation reviewer (4.8) would wave through as normal, and disagreements between the two are a useful signal.
...
RDIR=".self-review/$(git rev-parse --abbrev-ref HEAD | tr '/' '-')/opus-4-6"
mkdir -p "$RDIR"
...
echo "scope=DEEP branch=$BRANCH base=$BASE claude=$CLAUDE_BIN"
...
```bash
RDIR=".self-review/$(git rev-parse --abbrev-ref HEAD | tr '/' '-')/opus-4-6"; source "$RDIR/scope.env"
...
Launch a fresh Claude (Opus 4.6), read-only, `--effort max`; report to a file. **Large timeout.**

```bash
RDIR=".self-review/$(git rev-parse --abbrev-ref HEAD | tr '/' '-')/opus-4-6"; MODEL="claude-opus-4-6"; source "$RDIR/scope.env"
...
Print a `## Self-review summary (Opus 4.6, DEEP)` section:
...
- Verification: EditMode test result (e.g. `total=66 passed=65 skipped=1`) — honest; or "skipped — docs/php-only".

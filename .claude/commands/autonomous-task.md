...
```
Green = `failed="0"` with a non-zero `passed` (the suite is currently **126 deterministic tests + 2 `[Category("Live")]`** live tests that run in the default headless suite and need network). Fix to green before continuing.

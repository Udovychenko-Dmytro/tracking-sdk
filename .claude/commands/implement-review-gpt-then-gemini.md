...
Green = `failed="0"` with a non-zero `passed` in the `<test-run>` line (the suite is currently **126 deterministic tests + 2 `[Category("Live")]` live tests** that run in the default headless suite and need network). Any failure or compile error → fix to green. Report failures honestly (Fix and report rule) — never silently fix.

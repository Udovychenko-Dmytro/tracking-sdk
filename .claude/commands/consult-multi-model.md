...
4. **Verify** — run the headless Unity EditMode test run, but **only** if `.cs`/`.asmdef`/`.asmref` changed; skip for docs/PHP/asset-only diffs. The Unity Editor must be closed (single-instance lock). Set a large Bash timeout (up to `600000` ms):
   ...
   Green = `failed="0"` with non-zero `passed` (suite is currently 126 deterministic tests + 2 `[Category("Live")]` live tests that run in the default headless suite and need network). Failure — fix to green and report honestly (per the **Fix and report code problems** rule, never silently fix).
